...
@test "should drop a database and a user" {
  ...
  assert_line --regexp \
    "wrote .tmp/${RELEASE_NAME}/managed-pg/templates/job-drop-db-user.yml"
  assert_success
  ...
  run kubectl wait --for=condition=complete --timeout=5m "job.batch/${RELEASE_NAME}-managed-pg-drop-db-user"
  echo "" >&2
